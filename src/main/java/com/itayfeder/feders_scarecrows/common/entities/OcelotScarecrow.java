...
public class OcelotScarecrow extends TemplateScarecrow {
    private static final Predicate<Entity> ENTITY_PREDICATE = EntitySelector.NO_SPECTATORS.and((entity) -> {
        if (entity instanceof LivingEntity) {
            return ModTags.CREEPERS.contains(entity.getType());
        }
        return false;
    });

    public OcelotScarecrow(Level p_31556_, double p_31557_, double p_31558_, double p_31559_) {
        super(EntityTypeInit.OCELOT_SCARECROW, p_31556_);
        this.setPos(p_31557_, p_31558_, p_31559_);
    }

    public OcelotScarecrow(EntityType<OcelotScarecrow> scarecrowEntityType, Level level) {
        super(scarecrowEntityType, level);
    }

    public ItemStack getPickResult() {
        return new ItemStack(ItemInit.OCELOT_SCARECROW);
    }
